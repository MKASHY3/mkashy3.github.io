...
async function renderSecondChart() {
    const margin = {top: 10, right: 20, bottom: 30, left: 50},
        width = 1000 - margin.left - margin.right,
        height = 800 - margin.top - margin.bottom;
    const data = await d3.csv("https://mkashy3.github.io/data/3-productivity-vs-annual-hours-worked.csv");
    const year = 2015
    const filteredData = data.filter(function (d) {
...
async function renderThirdChart() {
    ...
    const data = await d3.csv("https://mkashy3.github.io/data/4-labor-productivity-per-hour-PennWorldTable.csv");
...
// Load external data and boot
    Promise.all([
        ...
        d3.csv("https://mkashy3.github.io/data/1-annual-working-hours-vs-gdp-per-capita-pwt.csv", function (d) {
            if (d.year == 2015) {
                data.set(d.code,
                    {
                        year: d.year,
